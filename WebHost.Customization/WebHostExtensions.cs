                    ...
                    else
                    {
                        ...
                        var retry = Policy.Handle<MySqlException>()
                            .WaitAndRetry(
                                ...
                                onRetry: (exception, timeSpan, retriedTimes, ctx) =>
                                {
                                    logger.LogWarning(exception, "[{prefix}] Exception {ExceptionType} with message {Message} detected on attempt {retry} of {retries}", nameof(TContext), exception.GetType().Name, exception.Message, retriedTimes, retries);
                                });

                        // If the mysql container is not created on run docker compose this
