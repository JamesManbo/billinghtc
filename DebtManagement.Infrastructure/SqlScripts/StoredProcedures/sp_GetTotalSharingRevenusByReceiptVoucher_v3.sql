CREATE PROCEDURE `sp_GetTotalSharingRevenusByReceiptVoucher` ( IN skip INT, IN inContractId INT, IN startDate DATE, IN endDate DATE, OUT total INT ) BEGIN
	SELECT
		t1.CurrencyUnitCode,
		t4.ReceiptVoucherId,
		t5.VoucherCode,
	...
	CASE			
			WHEN t5.IsFirstVoucherOfContract THEN
			IFNULL( sPoint.InstallationFee, 0 ) * IFNULL( t1.InSharedInstallFeePercent, 0 )/ 100 ELSE 0 
	...
	CASE			
			WHEN t3.HasStartAndEndPoint = 1 THEN
			IF( t5.IsFirstVoucherOfContract = 1, IFNULL( ePoint.InstallationFee, 0 ), 0 ) * IFNULL( t1.OutSharedInstallFeePercent, 0 )/ 100 
			ELSE t3.InstallationFee * IFNULL( t1.OutSharedInstallFeePercent, 0 )/ 100 
		...
		IFNULL( sPoint.MonthlyCost, 0 ) * IFNULL( t1.InSharedPackagePercent, 0 )/ 100 AS SPointSharingPackageAmount,
	CASE			
			WHEN t3.HasStartAndEndPoint = 1 THEN
			IFNULL( ePoint.MonthlyCost, 0 ) * IFNULL( t1.OutSharedPackagePercent, 0 )/ 100 ELSE t3.PackagePrice * IFNULL( t1.OutSharedPackagePercent, 0 )/ 100 
	...
	WHERE
		...
		AND t5.IssuedDate BETWEEN startDate 
		AND endDate 
	...
	WHERE
		...
		AND t5.IssuedDate BETWEEN startDate 
		AND endDate 
		AND t1.InSharedPackagePercent + t1.InSharedInstallFeePercent + t1.OutSharedInstallFeePercent + t1.OutSharedPackagePercent > 0 INTO total;
