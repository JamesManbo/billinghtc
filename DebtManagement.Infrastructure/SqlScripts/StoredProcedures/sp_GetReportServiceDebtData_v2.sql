...
BEGIN
		...
		CREATE TEMPORARY TABLE ContractValue 
			SELECT
				...
				IFNULL(csrl.ServiceId,0) AS ServiceId ,
				sc.GrandTotal - SUM( IFNULL( csrl.SharedFixedAmount, 0 ) ) AS ContractValue ,
				sc.CurrencyUnitId,
				sc.CurrencyUnitCode
			FROM ITC_FBM_Contracts.OutContractServicePackages AS sc
			INNER JOIN ITC_FBM_Contracts.Contractor AS t1 ON t1.Id = sc.InContractId
			LEFT JOIN ITC_FBM_Contracts.ContractSharingRevenues AS csr ON sc.InContractId = csr.InContractId
			LEFT JOIN ITC_FBM_Contracts.ContractSharingRevenueLines AS csrl ON csrl.CsrId = csr.Id 
			WHERE
					IFNULL( csr.SharingType, 1 ) = 1 
			GROUP BY
				sc.IncontractId 
		UNION ALL
			SELECT
				csr.InContractId,
				IFNULL(csrl.ServiceId,0) AS ServiceId ,
				SUM( csrl.SharedPackagePercent * csp.PackagePrice * csp.TimeLine_PaymentPeriod / 100 ) AS ContractValue ,
				csr.CurrencyUnitId,
				csr.CurrencyUnitCode
			FROM				
				 ITC_FBM_Contracts.ContractSharingRevenues AS csr 
				INNER JOIN ITC_FBM_Contracts.ContractSharingRevenueLines AS csrl ON csrl.CsrId = csr.Id
				INNER JOIN ITC_FBM_Contracts.OutContractServicePackages AS csp ON csp.Id = csrl.OutContractPackageId 
			WHERE
				csr.SharingType IN ( 2, 3 ) 
			GROUP BY
				csr.InContractId 
		UNION ALL
			SELECT
				csr.InContractId,
				IFNULL(csrl.ServiceId,0) AS ServiceId ,
				SUM( csrl.SharedFixedAmount ) AS ContractValue ,
				csr.CurrencyUnitId,
				csr.CurrencyUnitCode
			FROM
				 ITC_FBM_Contracts.ContractSharingRevenues AS csr 
