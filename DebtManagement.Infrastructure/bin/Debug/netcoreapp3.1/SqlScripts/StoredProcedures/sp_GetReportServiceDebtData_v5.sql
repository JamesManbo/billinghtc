...
BEGIN
		...
		CREATE TEMPORARY TABLE Contractor SELECT DISTINCT
			...
			t3.ContractorGroupNames AS CategoryName,
			t3.ContractorIndustryNames AS IndustryName
			FROM
				...
				INNER JOIN ITC_FBM_Contracts.OutContractServicePackages AS t2 ON t2.InContractId= ict.Id
				LEFT JOIN ITC_FBM_Contracts.ContractorProperties AS t3 ON t3.ContractorId = ctor.Id
			WHERE 1 = 1
