...
{
    public class ApplicationUserIntegrationEventService : IApplicationUserIntegrationEventService
    {
        private readonly Func<DbConnection, IIntegrationEventLogService> _integrationEventLogServiceFactory;
        private readonly IEventBus _eventBus;
        private readonly ApplicationUserDbContext _applicationUserContext;
        private readonly IIntegrationEventLogService _eventLogService;
        private readonly ILogger<ApplicationUserIntegrationEventService> _logger;

        public ApplicationUserIntegrationEventService(
            Func<DbConnection, IIntegrationEventLogService> integrationEventLogServiceFactory,
            IEventBus eventBus,
            ApplicationUserDbContext applicationUserContext,
            ILogger<ApplicationUserIntegrationEventService> logger)
        {
            _integrationEventLogServiceFactory = integrationEventLogServiceFactory ??
                                                 throw new ArgumentNullException(nameof(integrationEventLogServiceFactory));
            _eventBus = eventBus ?? throw new ArgumentNullException(nameof(eventBus));
            _applicationUserContext = applicationUserContext ?? throw new ArgumentNullException(nameof(applicationUserContext));
            _eventLogService = _integrationEventLogServiceFactory(_applicationUserContext.Database.GetDbConnection());
        ...
        public async Task AddAndSaveEventAsync(IntegrationEvent evt)
        {
            ...
            await _eventLogService.SaveEventAsync(evt, _applicationUserContext.GetCurrentTransaction());
        }
    }
}