...
{
    public interface ICustomerStructureQueries
    {
        IPagedList<CustomerStructureDTO> GetList(RequestFilterModel filterModel);
        IEnumerable<SelectionItem> GetSelectionList();
        CustomerStructureDTO Find(int id);
        CustomerStructureDTO FindByUserId(int id);
        bool CheckExistName(int id, string name);
        bool CheckExistCode(int id, string code);
    }
    public class CustomerStructureQueries : QueryRepository<CustomerStructure, int>, ICustomerStructureQueries
    {
        public CustomerStructureQueries(ApplicationUserDbContext applicationUserDbContext) : base(applicationUserDbContext)
        {

        }
        public IPagedList<CustomerStructureDTO> GetList(RequestFilterModel filterModel)
        {
            var dapperExecution = BuildByTemplate<CustomerStructureDTO>(filterModel);
        ...
        public CustomerStructureDTO Find(int id)
        {
            var dapperExecution = BuildByTemplate<CustomerStructureDTO>();
            dapperExecution.SqlBuilder.Where("t1.Id = @id", new { id });
            return dapperExecution.ExecuteScalarQuery();
        }

        public CustomerStructureDTO FindByUserId(int id)
        {
            var dapperExecution = BuildByTemplate<CustomerStructureDTO>();
