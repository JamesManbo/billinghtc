...
{
    public class MultipleSuspendChannelCommandHandler :
        BaseMultipleTransactionHandler,
        IRequestHandler<MultipleSuspendChannelCommand, ActionResponse>
    {
        ...
        public MultipleSuspendChannelCommandHandler(
            IMediator mediator,
            ...
            IRadiusAndBrasManagementService radiusAndBrasManagementService) : base(mediator, transactionQueries, projectQueries, attachmentFileService, fileRepository, userIdentity, notificationGrpcService, userGrpcService, contractorQueries, config)
        {
        ...
        public async Task<ActionResponse> Handle(MultipleSuspendChannelCommand request,
            CancellationToken cancellationToken)
        {
            ...
            foreach (var channel in targetChannels)
            {
                var transaction = new Transaction(request)
                {
                    ...
                    ReasonType = TransactionReason.SuspensionOtherReason.Id,
                    Reason = "Tạm ngưng hàng loạt theo dự án"
                };
                ...
                var suspendChannelCmd = new CUTransactionServicePackageCommand()
                {
                    IsOld = false,
                    IsMultipleTransaction = true,
                    TransactionType = TransactionType.SuspendServicePackage.Id
                };

                suspendChannelCmd.Binding(channel);

                if (request.AutoConfirmation)
                {
                    suspendChannelCmd.TimeLine.SuspensionStartDate = DateTime.UtcNow.AddHours(7);
                }

                transaction.AddTransServicePackage(suspendChannelCmd);
            ...
            if (request.AutoConfirmation)
            {
                ...
                //actionResponse.CombineResponse(await ApproveTransactions(UserIdentity.UserName));
                if (!actionResponse.IsSuccess) return actionResponse;
            }
            await PushNotifyOrEmailToImplementers(TransactionType.SuspendServicePackage.Id, request);
            return actionResponse;
        }
    }
}
