CREATE PROCEDURE `GetListMasterCustomerReport`(
		IN `customerIds` MEDIUMTEXT,
		IN `outContractCode` MEDIUMTEXT,
		IN `macketId` INT,
		IN `yearReport` INT,
		IN `uSDRate` INT,
		IN `timeLineSignedStartDate` Datetime,
		IN `timeLineSignedEndDate` DateTime,
		IN `timeLineEffectiveStartDate` DateTime,
		IN `timeLineEffectiveEndDate` DateTime,
...
-- 		SET outContractCode = '%' + outContractCode + '%';
		...
		WHERE ct.TimeLine_Signed BETWEEN IFNULL(timeLineSignedStartDate,'1900-01-01') AND IFNULL(timeLineSignedEndDate,'2900-01-01')
			AND (customerIds = '' OR us.IdentityGuid = customerIds)
			AND ct.ContractCode LIKE CONCAT('%',outContractCode,'%')
		...
		SELECT
			...
			csp.InstallationAddress_StartPoint AS InstallationAddressStartPoint,
			csp.InstallationAddress_EndPoint AS InstallationAddressEndPoint,
			csp.InstallationAddress_Street AS InstallationAddressStreet,
			csp.ServiceName,
		...
		WHERE (csp.TimeLine_Effective IS NULL OR csp.TimeLine_Effective BETWEEN IFNULL(timeLineEffectiveStartDate,'1900-01-01') AND IFNULL(timeLineEffectiveEndDate,'2900-01-01') )
		;
