...
-- 		SET outContractCode = '%' + outContractCode + '%';
		...
		CREATE TEMPORARY TABLE revenue
			SELECT
			OutContractId,
			CONVERT ( SUM( IF ( YEAR ( IssuedDate ) < 2021, PaidTotal, 0 ) ), SIGNED ) AS LastYearAmount,
			CONVERT ( SUM( IF ( YEAR ( IssuedDate ) = 2021, ReductionFreeTotal + Discount_Amount + PromotionTotalAmount + DiscountAmountSuspendTotal, 0 ) ), SIGNED ) AS LostRevenue,
