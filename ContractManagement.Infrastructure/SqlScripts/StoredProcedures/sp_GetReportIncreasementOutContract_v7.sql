...
#Routine body goes here...		
		...
		CASE WHEN reportType = 1 THEN SUM(t3.PackagePrice * t1.TimeLine_RenewPeriod )
				ELSE SUM(t3.PackagePrice / DAY(LAST_DAY(t3.TimeLine_Effective))
	...
	#	CREATE TEMPORARY TABLE tbTransaction 
		...
		WHERE 1 = 1
			AND t1.Type = 1			
  		AND t1.EffectiveDate BETWEEN startEffectDate AND endEffectDate
 			AND (marketAreaId = 0 OR t1.MarketAreaId = marketAreaId)
	...
	FROM  (  SELECT t1.Id,
		...
		WHERE 1=1
			AND tb1.EffectiveDate BETWEEN startEffectDate AND endEffectDate
