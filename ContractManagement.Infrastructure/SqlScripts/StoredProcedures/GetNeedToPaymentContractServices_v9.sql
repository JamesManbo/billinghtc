...
BEGIN
	...
	BEGIN
			...
			WHERE  oc.IsDeleted = FALSE
				-- Loại bỏ những hợp đồng đã thanh lý và hủy
				AND oc.ContractStatusId NOT IN (5, 9)
				-- Truy vấn những kênh đang hoạt động hoặc 
				-- những kênh đã tạm ngưng nhưng trả sau(thời gian tạm ngưng nằm trong kỳ cước)
				AND (t2.StatusId IN (0, 5) OR (t2.StatusId = 1 AND t2.TimeLine_PaymentForm = 0))
				AND t2.TimeLine_NextBilling IS NOT NULL
				AND CURDATE() >= DATE(t2.TimeLine_NextBilling)
				-- Chỉ sinh phiếu hàng kỳ cho kênh có phương pháp tính cước cố định hàng tháng
	...
	pm.PromotionName AS PromotionName,
	pm.PromotionType AS PromotionType,
	pmd.Id AS PromotionDetailId,
	t2.Id AS OutContractServicePackageId,
	pfc.PromotionValue AS PromotionValue,
	pfc.PromotionValueType AS PromotionValueType,
	pfc.IsApplied AS IsApplied,
	pfc.NumberMonthApplied AS NumberMonthApplied
	...
	LEFT JOIN PromotionForContracts pfc ON pfc.OutContractServicePackageId = t2.Id AND pfc.IsDeleted = FALSE
	LEFT JOIN PromotionDetails pmd ON pmd.Id = pfc.PromotionDetailId  AND pmd.IsDeleted = FALSE
	LEFT JOIN Promotions pm ON pm.Id = pmd.PromotionId AND pm.IsDeleted = FALSE;
END