CREATE PROCEDURE `sp_GetTotalDataIncontractFeeSharingReport`(
	IN marketAreaId int,
...
BEGIN
SELECT SUM(ContractRevenue) as ContractRevenueTotal, 
		SUM(TotalShareAmount) as ShareAllTotal,
		SUM(ShareChannelRentalAmount) as ShareChannelRentalTotal,
		SUM(ShareCommissionAmount) as ShareCommissionTotal,
		SUM(ShareRevenueAmount) as ShareRevenueTotal,
		SUM(ShareMaintenanceAmount) as ShareMaintenanceTotal,
		SUM(ShareInfrastructureAmount) as ShareInfrastructureTotal,
		SUM(ShareConstructionAmount) as ShareConstructionTotal,
		SUM(ShareEquipmentAmount) as ShareEquipmentTotal,
		SUM(ShareSupplyAmount) as ShareSupplyTotal,
		SUM(ShareCostOfAcceptanceAmount) as ShareCostOfAcceptanceTotal
