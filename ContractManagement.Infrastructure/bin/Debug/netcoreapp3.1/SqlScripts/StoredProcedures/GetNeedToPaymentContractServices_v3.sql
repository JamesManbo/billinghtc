...
BEGIN
	...
	BEGIN
			...
			WHERE  oc.IsDeleted = FALSE
				...
				AND (t2.StatusId IN (0, 5) OR (t2.StatusId = 1 AND DATE(t2.TimeLine_SuspensionStartDate) > DATE(IFNULL(t2.TimeLine_LatestBilling, t2.TimeLine_StartBilling))))
				AND t2.TimeLine_StartBilling IS NOT NULL
				AND t2.TimeLine_StartBilling < CURDATE()
				AND t2.TimeLine_NextBilling IS NOT NULL
				AND DATEDIFF(t2.TimeLine_NextBilling, CURDATE()) = 0
